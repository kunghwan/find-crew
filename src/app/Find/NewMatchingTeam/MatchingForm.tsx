  ...
  return (
    <form className="col gap-y-2.5 max-w-100 mx-auto p-5">
                ...
                if (key === "Enter" || key === "Tab") {
                  if (!nativeEvent.isComposing) {
                    try {
                      ...
                      if (!data) {
                        return alert("맴버 존재 x");
                      }
                      if (data[0]) {
                        if (confirm(`${data[0].name} 님이 맞음?`)) {
      ...
      <div className="row gap-x-2.5 mt-2.5">
        ...
        <button type="button" className=" flex-2 primary">
          {payload ? "수정" : "등록"}
        </button>
      </div>
    </form>
